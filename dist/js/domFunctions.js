...
const buildScreenReaderWeather = (weatherJson, locationObj) => {
	...
	return `${weatherJson.current.weather[0].description} 
		and ${Math.round(Number(weatherJson.current.temp))}&deg
...
const createCurrentConditionsDivs = (weatherObj, unit) => {
	...
	const maxTemp = createElem(
		'div', 
		'maxTemp', 
		`Hight ${Math.round(Number(weatherObj.daily[0].temp.max))}°`
		);
...
const translateIconToFontAwesome = (icon) => {
	...
	switch(firstTwoChars){
		...
		case '10':
			...
			break;
		case '11':
			i.classList.add('fas', 'fa-cloud-storm');
			break;
		case '13':
			i.classList.add('far', 'fa-snow-rain');
			break;
		case '50':
...
const displayDailyForecast = (dfArray) => {
	...
	const dailyForecastContainer = document.querySelector('#dailyForecast__contents');
